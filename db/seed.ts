...
const DEFAULT_EMAIL = 'admin@grandegordo.com';
...
async function main() {
  ...
  const email = positionalArgs[0] ?? DEFAULT_EMAIL;
  const password = positionalArgs[1] ?? DEFAULT_PASSWORD;
  const name = positionalArgs[2] ?? DEFAULT_NAME;
...
`.trim();
  ...
  console.log(
    `Admin seeded in ${target} D1 database "${databaseName}": ${email} / ${password}`,
  );
}
