...
dashboardRoutes.get("/client", async (c) => {
  ...
  try {
    ...
    const brandReadiness = computeBrandAssetReadinessFromKeys(
      brandAssetsResult.map((asset) => asset.r2Key),
    );
    const datasetStatus = await syncClientDatasetStatus(
      db,
      clientId,
      clientRecord.datasetStatus as ClientDatasetStatus,
    );
    ...
    return c.json({
      ...
      brandReadiness: {
        ready: brandReadiness.ready,
        missing: brandReadiness.missing,
      },
    });
  } catch (err) {
    console.error("Dashboard /client error:", err);
    return c.json(
      { error: "Dashboard client failed", details: String(err) },
      500,
    );
  }
});
...
dashboardRoutes.get("/stats", async (c) => {
  ...
  try {
    ...
    const reviewWindowEndMs = nowMs + 14 * 24 * 60 * 60 * 1000;
    ...
    const [
      ...
      db
        .select({
          ...
          completedThisMonth: sql<number>`cast(coalesce(sum(case when ${schema.jobs.status} = 'completed' and ${schema.jobs.updatedAt} >= ${monthStartMs} then 1 else 0 end), 0) as integer)`,
          deliveredThisMonth: sql<number>`cast(coalesce(sum(case when ${schema.jobs.status} = 'delivered' and ${schema.jobs.updatedAt} >= ${monthStartMs} then 1 else 0 end), 0) as integer)`,
          totalClients: sql<number>`cast((select count(*) from ${schema.clients}) as integer)`,
        })
        .from(schema.jobs),
      db
        .select({
          unitsPlannedThisMonth: sql<number>`cast(coalesce(sum(case when ${schema.jobs.createdAt} >= ${monthStartMs} then ${schema.jobs.unitsPlanned} else 0 end), 0) as integer)`,
          unitsConsumedThisMonth: sql<number>`cast(coalesce(sum(case when ${schema.jobs.createdAt} >= ${monthStartMs} then ${schema.jobs.unitsConsumed} else 0 end), 0) as integer)`,
      ...
      db
        .select({
          reviewsDueSoon: sql<number>`cast(coalesce(sum(case when ${schema.clients.nextReviewAt} >= ${nowMs} and ${schema.clients.nextReviewAt} <= ${reviewWindowEndMs} then 1 else 0 end), 0) as integer)`,
        })
        .from(schema.clients),
      // Capacity summary
      db
      ...
      db
        .select({
          pastDueJobs: sql<number>`cast(coalesce(sum(case when ${schema.jobs.dueAt} < ${nowMs} and ${schema.jobs.status} in ('pending', 'processing') then 1 else 0 end), 0) as integer)`,
      ...
      db
        ...
        .where(
          and(
            ...
            lte(schema.clients.nextReviewAt, new Date(reviewWindowEndMs)),
          ),
        )
    ...
    return c.json(
      { error: "Dashboard stats failed", details: String(err) },
      500,
    );
  }
});
